...
class CFRPlusDense:
    """Exact CFR+ (alternating updates, linear averaging, regret floor)."""
    ...
    def _update_strategy_for_player(self, player: int) -> None:
        for hid in range(self.H):
            if self.legal_counts[hid] <= 0:
                continue
            if int(self.popcount[hid] & 1) != player:
                continue
            regrets = self.R0[hid] if player == 0 else self.R1[hid]
    ...
    def _terminal_utility(
        ...
        *,
        ...
        A = self.A0 if player == 0 else self.A1
    ...
    def iterate(self) -> None:
        ...
        self._update_strategy_for_player(0)
        self._recompute_likelihoods()
        self._update_player(0, weight=weight)

        self._update_strategy_for_player(1)
        self._recompute_likelihoods()
        self._update_player(1, weight=weight)
