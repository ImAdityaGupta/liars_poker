...
class GPUDeepCFRTraverser:
    """Tensorized external-sampling traversal with device-resident records."""
    ...
    def __init__(self, trainer) -> None:
        ...
        self.claim_kind = torch.tensor(kind_codes, dtype=torch.long, device=self.device)
    ...
    def _terminal_values(
        ...
        *,
        ...
        row = torch.arange(len(last_claim), dtype=torch.long, device=self.device)
    ...
    def run_traversals(self, traverser: int, batch_size: int) -> Dict[str, int]:
        ...
        for depth in range(self.k + 1):
            ...
            strategy = self._strategy_batch(actor, features, legal_mask)
        ...
        advantage_features: List[torch.Tensor] = []
        advantage_targets: List[torch.Tensor] = []
        advantage_masks: List[torch.Tensor] = []
        ...
        for layer in reversed(layers):
            ...
            targets = (action_values - node_values[:, None]) * legal_mask
            advantage_features.append(layer["features"])
            advantage_targets.append(targets)
            advantage_masks.append(legal_mask)
            next_values = node_values

        advantage_count = 0
        if advantage_features:
            features = torch.cat(advantage_features, dim=0)
            targets = torch.cat(advantage_targets, dim=0)
            masks = torch.cat(advantage_masks, dim=0)
            advantage_count = int(features.shape[0])
        ...
        return {
            "advantage_records": advantage_count,
