...
def save_cfr_plus_run(
    ...
    *,
    ...
    state_path = run_dir / "cfr_plus_state.npz"
    np.savez_compressed(
        state_path,
        R0=cfr.R0,
        R1=cfr.R1,
...
def load_cfr_plus_state(run_dir: str | Path) -> Tuple[CFRPlusDense, GameSpec, Dict[str, object], int]:
    ...
    state_path = run_path / "cfr_plus_state.npz"
...
def cfr_plus_loop(
    ...
    iterations: int,
    *,
    cfr: CFRPlusDense | None = None,
    start_iter: int = 0,
    eval_every: int = 0,
    debug: bool = False,
) -> Tuple[DenseTabularPolicy, Dict[str, object], CFRPlusDense]:
    if cfr is None:
        cfr = CFRPlusDense(spec)
    ...
    for i in range(iterations):
        ...
        elapsed = time.perf_counter() - start
        print(f"[cfr+] iter={iter_idx} elapsed={elapsed:.2f}s")
...
def cfr_plus_resume(
    ...
    *,
    ...
    eval_every: int = 0,
    debug: bool = False,
) -> Tuple[DenseTabularPolicy, Dict[str, object], CFRPlusDense, GameSpec, int]:
    cfr, spec, metrics, iter_done = load_cfr_plus_state(run_dir)

    base_series = list(metrics.get("exploitability_series", []) or [])
    avg_policy, new_logs, cfr = cfr_plus_loop(
        spec=spec,
